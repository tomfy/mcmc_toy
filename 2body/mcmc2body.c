...
void print_state_walker_order(int n_Ts, int n_d, double** xs); 
void print_states_T_order(int n_Ts, int n_d, double** xs, int* Lws, int* Rws); // first L states, cold to hot, then R states
void new_print_states_T_order(chain_state* state);
// double pi(int n_dims, int n_peaks, double** peak_positions, double* heights, double* sigmas, double* x);
double pi(peaks* the_peaks, double* x);
// void update_x(int n_dims, int n_peaks, double** peak_positions, double* heights, double* sigmas, int iw, double* pis, double** xs, double Tinverse);
// void update_x(int n_dims, peaks* the_peaks, int iw, double* pis, double** xs, double Tinverse);
void new_update_x(int n_dims, peaks* the_peaks,  int iw, chain_state* state, double Tinverse);
// void old_T_swap(int n_Ts, double* pis, double** xs,  int* Ts, int* LRs, int* Lws, int* Rws, double* inverse_Temperatures);
void new_T_swap(chain_state* state, double* inverse_Temperatures);
...
int main(int argc, char* argv[]){
  ...
  // ***** Set up set of walkers *****
  // int n_walkers = n_Ts*2;
  /* double** xs = (double**)malloc(n_walkers*sizeof(double*)); // array of positions of walkers, indices are walker indices */
  /* double* pis = (double*)malloc(n_walkers*sizeof(double)); // array of current values of target density. */
  /* int* Ts = (int*)malloc(n_walkers*sizeof(int)); // array of Temperature indices */
  /* int* LRs = (int*)malloc(n_walkers*sizeof(int)); // array of 0's and 1's (indicating L or R of the paired 2 bodies. */
  /* int* Lws = (int*)malloc(n_Ts*sizeof(int)); // Lws[0] is the walker index of the Left 0th T chain (i.e. cold chain) */
  /* int* Rws = (int*)malloc(n_Ts*sizeof(int)); // Rws[0] is the walker index of the Right 0th T chain (i.e. cold chain) */
 ...
 chain_state* the_chain_state = (chain_state*)malloc(sizeof(chain_state));
  ...
  /*   double* point = (double*)malloc(n_dims*sizeof(double)); */
  /*   for(int j=0; j < n_dims; j++){ */
  /*     double r = gsl_rng_uniform(g_rng); */
  /*     point[j] = (2.0*r - 1.0)*init_width; */
  /*   } */
  /*   xs[i] = point; */
  /*   pis[i] = */
  /*     // pi(n_dims, n_peaks, peak_positions, heights, sigmas, point); */
  /*     //    double  pi  =  */
  /*     pi(the_peaks, point); */
  /*   // pis[i] = pi; */
  /*   //   printf("%g %g \n", pis[i], pi); */
  /*   Ts[i] = i / 2; //  n_Ts; // initially e.g. (n_Ts = 3:  Ts = (0,0,1,1,2,2) */
  /*   LRs[i] =  i % 2; //  n_Ts; // initially e.g. (n_Ts = 3:  LRs = (0,1,0,1,0,1)     */
  /*   if(LRs[i] == 0){ */
  /*     Lws[Ts[i]] = i; */
  /*   }else if(LRs[i] == 1){ */
  /*     Rws[Ts[i]] = i; */
  ...
  /* // ***** print info on starting conditions ***** */
  /* printf("# walker index  T index   T     LR     init X         init pi: \n"); */
  /* for(int i=0; i < n_walkers; i++){ */
  /*   printf("# %5i        %5i   %6.3f  %3i    ", i, Ts[i], Temperatures[Ts[i]], LRs[i]); */
  /*   for(int j=0; j < n_dims; j++){ */
  /*     printf("%4.2f ", xs[i][j]); */
  /*   }printf("%12.5g \n",  */
  /*           // pi(n_dims, n_peaks, peak_positions, heights, sigmas, xs[i] ) */
  /*           pi(the_peaks, xs[i]) */
  /*           ); */
  /* }   */

  /* }else{ */
  // set up initial states of walkers (chain_state struct version)
    //  chain_state* the_chain_state = (chain_state*)malloc(sizeof(chain_state));
  ...
  for(int i = 0; i < n_updates; i++){ // loop through updates
    ...
    /*   //    printf("%i  %i  %5.3f \n", j, Ts[j], Temperatures[Ts[j]]); */
    /*   //xs[j] =  */
    /*   check_state_consistency(n_dims, n_Ts, pis, xs, Ts, LRs, Lws, Rws); */
    /*   //  update_x(n_dims, n_peaks, peak_positions, heights, sigmas, j, pis, xs, inverse_Temperatures[Ts[j]]); */
    /*      update_x(n_dims, the_peaks, j, pis, xs, inverse_Temperatures[Ts[j]]); */
    /*      // new_update_x(n_dims, the_peaks, j, the_chain_state, inverse_Temperatures[the_chain_state->w_ts[j]]); */
    /*   //   print_states_walker_order(n_Ts, n_dims, xs); */
    /*   check_state_consistency(n_dims, n_Ts, pis, xs, Ts, LRs, Lws, Rws); */
    /* } */
    /* // T-swapping;  update temperatures of walkers. */
    /* check_state_consistency(n_dims, n_Ts, pis, xs, Ts, LRs, Lws, Rws); */
    /* old_T_swap(n_Ts, pis, xs, Ts, LRs, Lws, Rws, inverse_Temperatures); */
    /* //new_T_swap(the_chain_state, inverse_Temperatures); */
    /* check_state_consistency(n_dims, n_Ts, pis, xs, Ts, LRs, Lws, Rws); */
    /* printf("%5i ", i); */
    /* print_states_T_order(n_Ts, n_dims, xs, Lws, Rws); */
    /* printf("\n"); */
    /* }else{ // new  */
 for(int j = 0; j < n_walkers; j++){ // update positions of walkers
      //    printf("%i  %i  %5.3f \n", j, Ts[j], Temperatures[Ts[j]]);
 ...
 //    check_state_consistency(n_dims, n_Ts, pis, xs, Ts, LRs, Lws, Rws);
    ...
    //   check_state_consistency(n_dims, n_Ts, pis, xs, Ts, LRs, Lws, Rws);
    printf("%5i ", i);
    new_print_states_T_order(the_chain_state);
    printf("\n");
    }
...
void print_states_walker_order(int n_Ts, int n_d, double** xs, int* Ts){
  int n_w = 2*n_Ts;
...
void print_states_T_order(int n_Ts, int n_d, double** xs, int* Lws, int* Rws){
...
/*   double pi_x = pis[iw]; // pi(n_dims, n_peaks, peak_positions, heights, sigmas, x); */
/*   //  printf("after pi_x = pis[iw]. n_dims: %i \n", n_dims); */

/*   double* prop_x = (double*)malloc(n_dims*sizeof(double)); */
/*   //  printf("after alloc prop_x \n"); fflush(stdout); */
/*   for(int i = 0; i < n_dims; i++){ */
/*     prop_x[i] = xs[iw][i] + prop_w * (gsl_rng_uniform(g_rng) - 0.5); */
/*   } */
...
/* void old_T_swap(int n_Ts, double* pis, double** xs,  int* Ts, int* LRs, int* Lws, int* Rws, double* inverse_Temperatures){ */
/*   //  printf("top of T_swap\n"); */

/*   for(int i=1; i<n_Ts; i++){ // T-swapping among Ls */
/*     double Tc_inverse = // 1.0/Temperatures[i-1]; //  */
/*       inverse_Temperatures[i-1]; */
/*     double Th_inverse = // 1.0/Temperatures[i]; //  */
/*       inverse_Temperatures[i]; */
/*     { */
/*       // first the 'left' set of walkers: */
/*       int iw_hot = Lws[i]; // index of walker with ith temperature */
/*       int iw_cold = Lws[i-1]; // index of walker with (i-1)th, i.e. next colder, temperature */
/*       double pi_cold = pis[iw_cold]; */
/*       double pi_hot = pis[iw_hot]; */
/*       if(  */
/*          (pi_hot > pi_cold)  */
/*          ||  */
...
/*         tmp = Ts[iw_hot]; */
/*         Ts[iw_hot] = Ts[iw_cold]; */
/*         Ts[iw_cold] = tmp; */
/*       } */
/*     } */
/*     { */
/*       // then the 'right' set of walkers: */
/*       int iw_hot = Rws[i]; // index of walker with ith temperature */
/*       int iw_cold = Rws[i-1]; // index of walker with (i-1)th, i.e. next colder, temperature */
/*       double pi_cold = pis[iw_cold]; */
/*       double pi_hot = pis[iw_hot]; */
/*       if(  */
/*          (pi_hot > pi_cold)  */
