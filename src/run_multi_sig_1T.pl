...
while ($F_sigma < $F_sigma_max*$f_sigma) {
   ...
   my $prop_sig_2 = 2.38*$target_peak_sigma/sqrt($n_dimensions)*$F_sigma; # 'opt' within peak sigma for T = 1
   my $prop_p1 = 0.0;
 ...
 #  print "temperatures: ", join(", ", @temperatures), "\n";
   ...
   if ($proposals_string) {
      ...
      if ($n_temperatures == 1) {
         my $propstr = "gaussian  $prop_sig_1  " . $prop_sig_2*sqrt($temperatures[0]) . "  0"; # . 1.0/($cool_rate+1.0);
...
sub trials{                  # do multiple runs with same parameters, 
   ...
   for (1..$n_reps) {
      ...
      store_accept_info('accept_info', \%accept_info, $param_val);
...
sub store_accept_info{
   ...
   my $key = shift || undef; # could be T or sigma_prop or ...
   my @accept_info_lines = split("\n", $accept_info_string);
   for my $a_line (@accept_info_lines) {
      my @cols = split(" ", $a_line);
      $key = $cols[2] if(!defined $key);
      $accept_info->{'ntry2'}->{$key} += $cols[5];
      $accept_info->{'nacc2'}->{$key} += $cols[6];
      if (scalar @cols >= 9) {
         $accept_info->{'nswaptry'}->{$key} += $cols[7];
         $accept_info->{'nswapacc'}->{$key} += $cols[8];
      }
   }
}
